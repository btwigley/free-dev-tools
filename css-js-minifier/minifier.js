/**
 * CSS/JS Minifier - Client-Side Tool
 * Wigley Studios Free Resources
 *
 * Uses csso (CSS), Terser (JS), and JSZip for download.
...
(function () {
    ...
    // ── State ───────────────────────────────────────────────────────────────────
    let minifiedFiles = []; // { name, type, original, minified, originalSize, minifiedSize }
    ...
    function minifyCSS(code) {
        try {
            // csso is exposed as a global from the CDN bundle
            if (typeof csso === 'undefined') {
                throw new Error('csso library not loaded. Please refresh the page.');
            }
    ...
    async function minifyJS(code) {
        try {
            // Terser is exposed as a global from the CDN bundle
    ...
    async function processFiles(fileList) {
        ...
        for (const file of validFiles) {
            ...
            if (result.success) {
                ...
                // Show error inline in table as a failed row
                minifiedFiles.push({
                    name: file.name,
                    type: ext,
                    original: content,
        ...
        minifiedFiles.forEach(function (file, idx) {
            ...
            if (file.error) {
                tr.innerHTML =
                    ...
                    '<td colspan="2" style="color: #fca5a5;">Error: ' + escapeHtml(file.error) + '</td>' +
                    '<td></td>';
            } else {
    ...
    window.downloadAll = async function () {
        ...
        minifiedFiles.forEach(function (file) {
            if (!file.error && file.minified) {
                zip.file(file.name, file.minified);
            }
        });
    ...
    window.minifyPaste = async function () {
        var input = pasteInput.value.trim();
        hideError(pasteError);
        pasteOutput.value = '';

        if (!input) {
            showError(pasteError, 'Please paste some code first.');
            return;
        }

        var result = await minifyContent(input, currentPasteType);
        if (result.success) {
            pasteOutput.value = result.code;
        } else {
            showError(pasteError, 'Minification error: ' + result.error);
        }
    };
    ...
    window.copyOutput = function () {
        ...
        navigator.clipboard.writeText(text).then(function () {
            ...
            console.warn('[minifier] clipboard copy failed', err);
        });
    };
    ...
    window.switchPasteType = function (type) {
        ...
        pasteInput.placeholder = type === 'css'
